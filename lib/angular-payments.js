...
.directive('stripeFormBtc', ['$window', '$parse', 'Common', function($window, $parse, Common) {
    
  // directive intercepts form-submission, obtains Stripe's Bitcoin receiver using stripe.js
  // and then passes that to callback provided in stripeFormBTC, attribute.
...
console.log("HERE00");
  ...
  _getDataToSend = function(data){
    ...
    var possibleKeys = ['payment_amount', 'currency', 'description', 'email']
    ...
    for(i in possibleKeys){
        if(data.hasOwnProperty(possibleKeys[i])){
          if(possibleKeys[i] == 'payment_amount') {
            ret['amount'] = angular.copy(data[possibleKeys[i]].amount + "00");
          } else {
            ret[camelToSnake(possibleKeys[i])] = angular.copy(data[possibleKeys[i]]);
          }
        }
...
console.log("HERE15");
      var form = angular.element(elem);
      var btcReceiver = form.find('btc-receiver');
console.log("HERE20");
console.dir(btcReceiver);
      scope.$watch('formData.paymentSubmitFormBTC.payment_amount', function() {
        console.log("HERE25");
        if(form.hasClass('ng-valid')) {
          $window.Stripe.bitcoinReceiver.createReceiver(_getDataToSend(scope.formData.paymentSubmitFormBTC), function(error, response) {
            var args = arguments;
            console.log('response: ');
            console.dir(elem[0].querySelector('a[name=scan-code]'));
            scope.$apply(function() {
              console.dir(scope);
              elem[0].querySelector('input[name=btc-receiver]').value = response.inbound_address;
              elem[0].querySelector('a[name=open-wallet]').href = response.bitcoin_uri;
              console.log("AALIYAH");
              console.dir(elem[0].querySelector('a[name=scan-code]'));
              elem[0].querySelector('a[name=scan-code]').setAttribute('june07-add-qr-code', response.bitcoin_uri);
              scope.formData.paymentSubmitFormBTC.bitcoinReciever = response.bitcoin_uri;
              console.dir(elem[0].querySelector('a[name=scan-code]'));
              scope[attr.stripeFormBtc].apply(scope, args);
            });
          });
        } else {
          scope.$apply(function() {
            scope[attr.stripeFormBtc].apply(scope, [400, {error: 'Invalid form submitted.'}]);
          });
          button.prop('disabled', false);
        }
